/**
 * Usage Tracking Middleware for NexusDoc
 *
...
function detectOperation(req: Request): string {
  ...
  // NexusDoc Medical AI specific operations
  if (path.includes('/consultation')) return 'clinical_consultation';
  if (path.includes('/imaging')) return 'medical_imaging';
  if (path.includes('/diagnosis')) return 'diagnosis_assist';
  if (path.includes('/drug-discovery')) return 'drug_discovery';
  if (path.includes('/telemedicine')) return 'telemedicine';
  if (path.includes('/soap')) return 'soap_note_generation';
  if (path.includes('/coding')) return 'medical_coding';
  if (path.includes('/trial')) return 'clinical_trial_match';
  if (path.includes('/risk')) return 'risk_stratification';
  if (path.includes('/guideline')) return 'clinical_guideline';
  if (path.includes('/pubmed')) return 'pubmed_search';
  if (path.includes('/drug-safety')) return 'drug_safety_check';
  if (path.includes('/explain')) return 'xai_explanation';
  if (path.includes('/agent')) return 'agent_spawn';
  if (path.includes('/hipaa')) return 'hipaa_compliance';
  if (path.includes('/audit')) return 'audit_trail';
...
function isExemptPath(path: string): boolean {
  const exemptPaths = ['/health', '/healthz', '/ready', '/readiness', '/liveness', '/startup', '/metrics', '/ping', '/', '/version', '/api/doctor/health'];
